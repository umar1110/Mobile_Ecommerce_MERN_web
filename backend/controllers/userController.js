...
exports.loginUser = catchAsyncFuncError(async (req, res, next) => {
    ...
    if (!email || !password) {
        return next(new ErrorHandler("Please Enter Email & password both"))
            , 400
    } // bad request = 400
...
exports.forgetPassword = catchAsyncFuncError(async (req, res, next) => {
    ...
    const resetPasswordUrl = `${req.protocol}:://${req.get("host")}/password/reset/${resetToken}`  // wich pass through mail
...
exports.updatePassword = catchAsyncFuncError(async (req, res, next) => {
    ...
    if (!(req.body.oldPassword && req.body.newPassword && req.
    ...
    user.password = req.body.newPassword,
        await user.save(); // automaticallly run pre method in schema because password modified , and it will convert password to the bcrypt.hash()
...
exports.getSingleUser = catchAsyncFuncError(async (req, res, next) => {
    ...
    if (!user) {
        return next(new ErrorHandler(`User not exists with id ${req.params.id}`, 400))
    }
